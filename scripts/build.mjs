...
try {
  ...
  /* not a checkout yet; the version alone is enough to identify the build */
}

const stamp = (what) =>
  `/* ${what}\n` +
  ` * Generated by trev-ds v${pkg.version} (${sha}). Do not edit by hand:\n` +
  ` * change src/ in trev-delivers/trev-ds and re-run \`npm run build\`.\n` +
  ` */\n\n`;
...
writeFileSync(
  ...
  stamp("trev-ds base — shared element behaviours. Opt-in; import after primitives and a theme.") +
    readFileSync(join(src, "base.css"), "utf8").replace(/^\/\*[\s\S]*?\*\/\n+/, ""),
);

writeFileSync(
  join(dist, "css", "primitives.css"),
  stamp("trev-ds primitives — shared by every app, identical everywhere.") +
...
for (const theme of themes) {
  writeFileSync(
    ...
    stamp(`trev-ds theme: ${theme.label} — ${theme.description}`) + block(":root", themeVars(theme)),
  );
}

writeFileSync(
  join(dist, "css", "all-themes.css"),
  stamp("trev-ds — every theme, selectable with data-ds-theme. For the docs site; apps import their own theme file instead.") +
...
writeFileSync(
  ...
  stamp("trev-ds — Tailwind v4 bridge. @import this after tailwindcss, alongside your theme file.") +
